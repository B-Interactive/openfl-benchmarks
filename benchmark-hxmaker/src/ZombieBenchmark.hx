...
class ZombieBenchmark extends Stage {
	...
	override function onUpdate(dt:Float) {
		...
		if (mFrameTimeCount < BUFFER_SIZE)
			mFrameTimeCount++;
		var totalDelta:Float = 0;
		for (i in 0...mFrameTimeCount)
			totalDelta += mFrameTimes[i];
		...
		if (++mHudTick >= 6) {
			mHudTick = 0;
			if (mHud != null)
				mHud.data = "Zombies: " + mZombies.length + " | FPS: " + Std.int(mFps) + " | Draw Calls: " + ContextStats.drawCall;
		}
	}

	private function spawnZombies():Void {
		for (t in 0...TOTAL_ZOMBIE_TYPES) {
			if (mFrames[t] == null || mFrames[t].length == 0)
				continue;
